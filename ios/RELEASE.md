...
- [x] App ID に Game Center を有効化
- [x] App Store Connect でリーダーボードを作成
      - 名前 `High Score` ／ **ID `com.snaproom.dotdrop.highscore`**
      - 整数／自己ベストのみ／高いスコアが上位／日本語・英語の表示あり
      - バージョン1.0の審査対象に追加ずみ
- **ID はコード（`GameCenter.leaderboardID`）と一字一句同じにすること。**
  違っていても何のエラーも出ず、スコアが黙って捨てられる
- 「自己ベストのみ」なので、毎ゲーム送ってよい。Apple 側が一番良いものだけ残す
- リーダーボードを1つ作ると、友達への「チャレンジ」はコードなしで付いてくる

## アップロードのたびにビルド番号を上げる

`CURRENT_PROJECT_VERSION`（Xcode の Build）を +1 する。**同じ番号は2回受け付けられない。**
バージョン（`MARKETING_VERSION` / 1.0.0）は、審査に出す版が変わるまでそのまま。
