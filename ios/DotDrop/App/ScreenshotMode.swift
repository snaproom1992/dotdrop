...
enum ScreenshotMode {
    ...
    static func apply(to session: GameSession, name: String) {
        ...
        case "demo", "split":
            ...
            // 総当たりして選んだもの。**実機と同じ寸法で探すこと**（下の注意を参照）。
            // 種12・ステージ1（千鳥）で 127〜135度・強さ5 だと、玉が11個まで増え、
            // 1回92点（中央値）、100点超えが45%、6個以上に増えるのが7〜8割。
            session.startFreePlay()
            session.engine.boardSeed = 12
            session.engine.setLayout(0, animate: false)
    ...
    private static func playByItself(_ session: GameSession) {
        ...
        //
        // **探すときは実機と同じ寸法で回すこと。**本家の台は高さ700・発射140 だが、
        // アプリは HUD のぶん発射が161.5まで下がり、画面も縦に長い（754）。
        // 釘の行数も位置も変わるので、本家の寸法で選んだ角度はまったく当たらなかった。
        //
        // 強さの段（1〜5）の境目ちょうどだと1つ上に転びかねないので、わずかに内側にしてある
        let shots: [(Double, Double)] = [
            (-85.0, 97.7),    // 131° 強さ5　中央値79点・玉11・6個以上82%
            (-91.6, 91.6),    // 135° 強さ5　中央値92点・玉11・100点超え45%
            (-77.9, 103.4),   // 127° 強さ5　中央値63点・玉7
            (-50.8, 58.5),    // 131° 強さ3　中央値72点・玉9
            (-91.6, 91.6),    // 135° 強さ5
            (-54.8, 54.8),    // 135° 強さ3　中央値69点・玉9
            (-46.4, 120.9),   // 111° 強さ5　中央値64点・玉9
        ]
        Task { @MainActor in
            var i = 0
