...
final class GameAudio {
    ...
    enum Wave {
        ...
        func sample(_ phase: Double) -> Double {
            ...
            case .triangle:
                return abs((phase / .pi).truncatingRemainder(dividingBy: 2) - 1) * 2 - 1
            }
        }
    }
    ...
    func voice(freq: Double, dur: Double, gain: Double = 0.1, wave: Wave = .triangle, delay: Double = 0) {
        ...
        for (mul, g) in layers {
            ...
            for i in 0..<layerN {
                ...
                // 下の層はいつもサイン。形が付くのは基音だけ（本家と同じ）
                let raw = mul == 1 ? wave.sample(phase) : sin(phase)
                let sample = Float(raw * env)
                let idx = delayN + i
                if idx < Int(frames) {
    ...
    /// フィーバー突入。上へ駆け上がる7音。**これが鳴らないと入った実感がない**
    func playFever() {
        let base = note(1, fever: false)
        for (k, sp) in [0, 4, 7, 12, 16, 19, 24].enumerated() {
            voice(freq: base * pow(2.0, Double(sp) / 12.0),
                  dur: 0.4, gain: 0.07, wave: .square, delay: Double(k) * 0.06)
        }
    }
}
