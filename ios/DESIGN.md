...
| 順 | 内容 | 完了の目安 |
|----|------|------------|
| 0 | 本設計・空プロジェクト・フィクスチャ骨組み | 完了 |
| 1 | Engine 移植＋フィクスチャ一致 | 完了 |
| 2 | 受け皿・引っ張り（Web同等）・STAGE/FEVER・結果 | 実装済み（確認待ち） |
| 3 | 音・振動・演出（帯・数字ドン） | 手触りが Web に近づく |
| 4 | タイトル演出・あそびかた・保存・ランキング | ストア提出候補 |
| 5 | TestFlight → 審査 | 実機で作者確認 |
